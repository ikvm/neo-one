...
export class SourceMap {
  ...
  public getOriginalPosition(
    line: number,
    column: number,
  ): { readonly source: string | undefined; readonly line: number | undefined; readonly column: number | undefined } {
    const { line: l, column: c, source: s } = this.sourceMap.originalPositionFor({
      line,
      column,
    });

    return { line: l === null ? undefined : l, column: c === null ? undefined : c, source: s === null ? undefined : s };
  }
...
export function extractSourceMapUrl(fileUri: string, fileContents: string): Promise<string> {
